...
class ReportAdapter(
    ...
    private fun uniqueEmployeeReports(): List<ReportItem> {
        if (reportList.isEmpty()) return emptyList()

        val bestByEmployee = LinkedHashMap<String, ReportItem>()

        for (report in reportList) {
            val key = normalizedEmployeeName(report.employeeName)
            if (key.isEmpty()) continue

            val existing = bestByEmployee[key]

            if (existing == null) {
                bestByEmployee[key] = report
            } else {
                val replace = when {
                    report.assigned != existing.assigned ->
                        report.assigned > existing.assigned

                    report.resolved != existing.resolved ->
                        report.resolved > existing.resolved

                    report.successRate != existing.successRate ->
                        report.successRate > existing.successRate
    ...
    override fun getItemCount(): Int {
        val bodyCount = if (areaList.isNotEmpty()) {
            areaList.size
        } else {
            uniqueEmployeeReports().size
        }
    ...
    override fun onBindViewHolder(
        ...
        when (holder) {
            ...
            is ItemViewHolder -> {
                val employees = uniqueEmployeeReports()
    ...
    class HeaderViewHolder(itemView: View) :
        ...
        private val topEmployeeText: TextView =
            itemView.findViewById(R.id.topEmployeeText)

        private val overallSummaryText: TextView =
            itemView.findViewById(R.id.overallSummaryText)

        private val repeatReportText: TextView =
            itemView.findViewById(R.id.repeatReportText)

        private val areaReportText: TextView =
            itemView.findViewById(R.id.areaReportText)
        ...
        fun bind(
            ...
            overallSummaryText.visibility =
                if (data.summaryVisible) {
                    View.VISIBLE
                } else {
                    View.GONE
                }
            ...
            repeatReportText.visibility =
                if (data.repeatVisible) {
                    View.VISIBLE
                } else {
                    View.GONE
                }

            repeatReportText.setOnClickListener {
                onRepeatClick()
            }

            areaReportText.text = data.areaReportText
            areaReportText.visibility =
                if (data.areaReportVisible) {
                    View.VISIBLE
                } else {
                    View.GONE
                }
    ...
    class ItemViewHolder(itemView: View) :
        ...
        val starsText: TextView =
            itemView.findViewById(R.id.starsText)
        ...
        fun bind(report: ReportItem) {
            ...
            assignedText.text =
                "Total Assigned      : ${report.assigned}"
            ...
            repeatText.text =
                "Repeat Complaints   : ${report.repeatComplaints}"

            averageTimeText.text =
                "Avg Resolve Time    : ${report.averageTime}"
    ...
    class AreaViewHolder(itemView: View) :
        ...
        private val rankBadge: TextView =
            itemView.findViewById(R.id.rankBadge)

        private val areaNameText: TextView =
            itemView.findViewById(R.id.areaNameText)

        private val totalChip: TextView =
            itemView.findViewById(R.id.totalChip)

        private val resolvedChip: TextView =
            itemView.findViewById(R.id.resolvedChip)
        ...
        private val areaPercentText: TextView =
            itemView.findViewById(R.id.areaPercentText)
        ...
        fun bind(area: AreaReportItem) {
            ...
            areaPercentText.text =
                "${area.successRate}% Resolved"
        }
    }
}