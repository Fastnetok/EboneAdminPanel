...
class EboneAdminApp : Application() {
    ...
    override fun onCreate() {
        ...
        registerActivityLifecycleCallbacks(object : ActivityLifecycleCallbacks {
            override fun onActivityResumed(activity: Activity) {
                ...
                // Show any pending popups that couldn't show before
                if (pendingPopups.isNotEmpty()) {
                    val copy = pendingPopups.toList()
                    pendingPopups.clear()
                    copy.forEach { (name, reason, key) ->
                        showLeavePopup(name, reason, key)
                    }
                }
            }
    ...
    private fun showOrQueue(name: String, reason: String, key: String) {
        val activity = foregroundActivity
        if (activity != null && !activity.isFinishing) {
            showLeavePopup(name, reason, key)
        } else {
            // Queue it — will show when next activity resumes
