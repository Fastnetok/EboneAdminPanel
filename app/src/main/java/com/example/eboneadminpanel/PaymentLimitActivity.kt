...
class PaymentLimitActivity : BaseAdminActivity() {
    ...
    private fun loadLimits() {
        settingsRef.get()
            .addOnSuccessListener { snapshot ->
                ...
                statusText.text = "Current limit: Rs. $minimum  -  Rs. $maximum"
            }
    ...
    private fun saveLimits() {
        ...
        if (minimum == null || maximum == null) {
            Toast.makeText(this, "Dono limits mein valid amount likhein.", Toast.LENGTH_LONG).show()
            return
        }

        if (minimum < 1L) {
            Toast.makeText(this, "Minimum amount Rs. 1 se kam nahi ho sakta.", Toast.LENGTH_LONG).show()
            return
        }
